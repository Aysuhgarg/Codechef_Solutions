...
class Codechef
{
   ...
   /*boolean isCyclic(ArrayList<LinkedList<Integer>> l,int n,int s)
    {
        boolean visit[]=new boolean[n];
        int parent[]=new int parent[n];
        LinkedList<Integer> que=new LinkedList<Integer>();
        for(int i=0;i<n;i++)
        {
            visit[i]=false;
   ...
   void dfs_helper(int s,int visit[],List<LinkedList<Integer>> l,LinkedList<Integer> p)
    {
        visit[s]=true;
       // int n=l.get(s).get(0);
      ...
      {
        if(visit[i]!=true){
        dfs_helper(i,visit,l,p);}
      }
      p.add(s);
    }

   void dfs(List<LinkedList<Integer>> l,int v)
    {
